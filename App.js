   ...
   return (
      <View style={[StyleSheet.absoluteFillObject, styles.circleContainer]}>
         {data.map((item, index) => {
            ...
            return <Animated.View key={index} style={[styles.circle, { backgroundColor: item.color, opacity, transform: [{ scale }] }]} />
         })}
      </View>
   );
};
...
const Pagination = ({ scrollX }) => {
   ...
   return (
      <View style={styles.pagination}>
         <Animated.View style={[styles.paginationIndicator, { position: 'absolute', transform: [{ translateX }] }]} />
         {data.map(item => {
            return (
...
export default function App() {
   ...
   return (
      <View style={styles.container}>
         ...
         <Animated.FlatList keyExtractor={item => item.key} data={data} pagingEnabled showsHorizontalScrollIndicator={false} horizontal
            onScroll={Animated.event([{ nativeEvent: { contentOffset: { x: scrollX } } }], { useNativeDriver: true })} scrollEventThrottle={16}
            renderItem={item => {
               return <Item  {...item.item} index={item.index} scrollX={scrollX} />
            }} />
